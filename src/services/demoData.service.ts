...
export const seedDemoData = async (tenantId: string, userId: string): Promise<void> => {
    ...
    if (paquetesError || !paquetes || paquetes.length === 0) {
        console.error('[seedDemoData] Error creando paquetes demo:', paquetesError);
        return;
    }
    ...
    if (!maria || !carlos) {
        console.error('[seedDemoData] Error creando clientes demo');
        return;
    }

    const { data: pasajerosDemo } = await supabase
        .from('pasajeros')
        .insert([
    ...
    if (cotizacionesError || !cotizaciones) {
        console.error('[seedDemoData] Error creando cotizaciones demo:', cotizacionesError);
        return;
    }
    ...
    if (vinculos.length > 0) {
        await supabase.from('cotizacion_pasajeros').insert(vinculos);
    }
};
